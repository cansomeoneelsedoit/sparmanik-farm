...
def seed():
    ...
    try:
        ...
        # Parts (pre-load all 24 items — add any missing by name)
        PARTS_SEED = [
        ...
        existing_parts = {r[0] for r in db.execute(select(Part.name)).all()}
        added = 0
        ...
        if added:
            db.commit()
            print(f"  + {added} harvest parts added (total seed: {len(PARTS_SEED)})")
        else:
            print(f"  = all {len(PARTS_SEED)} harvest parts present")

        # Raw Materials (nutrient ingredients — add any missing by name)
        RAW_SEED = [
        ...
        existing_raw = {r[0] for r in db.execute(select(RawMaterial.name)).all()}
        added = 0
        ...
        if added:
            db.commit()
            print(f"  + {added} raw materials added (total seed: {len(RAW_SEED)})")
        else:
            print(f"  = all {len(RAW_SEED)} raw materials present")

        # Mixed Nutrients (ready-to-use products — add any missing by name)
        MIXED_SEED = [
        ...
        existing_mixed = {r[0] for r in db.execute(select(MixedNutrient.name)).all()}
        added = 0
        for name, unit, crop in MIXED_SEED:
            if name not in existing_mixed:
                db.add(MixedNutrient(name=name, unit=unit, crop=crop))
                added += 1
        if added:
            db.commit()
            print(f"  + {added} mixed nutrients added (total seed: {len(MIXED_SEED)})")
        else:
            print(f"  = all {len(MIXED_SEED)} mixed nutrients present")
